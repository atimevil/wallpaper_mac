...
- **"테스트 N개 통과"는 완료 근거가 아니다.** M4 Task 3에서 통과하던 162개 안에
  검증력이 0인 테스트가 섞여 있었다(`aliveCount <= maxCount`는 슬롯 수가 구조적
  상한이라 항상 참이라, 결함을 되살려도 통과했다). 새 테스트가 결함을 정말 잡는지는
  **구현을 일부러 망가뜨려 그 테스트가 실패하는 것을 확인**해야 알 수 있다.
  결함을 잡으려고 쓴 테스트는 이 변이 확인을 거치고, 실패 메시지를 보고에 남긴다.
- **브리프나 계획이 요구하는 게 불가능하면 멈추고 보고한다.** 단언을 통과하게
  바꾸지 마라. Task 3에서 "모든 파티클이 죽을 때까지 기다린다"가 연속 방출
  이미터에서 불가능했는데, 보고 없이 단언을 약화시켜 가짜 테스트가 됐다.
...
> **시간을 다루는 규약.** `update(deltaTime:)`은 한 번에 `maxTimeStep`(0.1초)을 넘겨
> 적분하지 않는다. 절전에서 깨어나면 수천 초가 들어오는데, 그걸 그대로 적분하면
> 파티클이 화면 밖으로 순간이동하고, 잘게 쪼개 따라잡으면 수만 번 반복하며 멈춘다.
> 그래서 **넘치는 시간은 버린다.** 따라서 테스트가 "1초를 돌린다"를 표현하려면
> 반드시 `maxTimeStep` 이하로 쪼개 넣어야 한다. `update(deltaTime: 1.0)` 한 번은
> 1초가 아니라 0.1초다.
>
> 쪼갤 때는 **2의 거듭제곱 분수**를 써라(1/16, 1/64). 1/60 같은 값은 이진수로
> 정확하지 않아 60번 더하면 1.0이 아니라 0.9999999999999998이 되고, 방출 누적의
> 정수부가 10이 아니라 9가 된다(실측). 개수를 정확히 단언하는 테스트가 이것 때문에
> 깨지면 원인을 시뮬레이션에서 찾게 된다.
...
3. **입력 위생.** 클램프 상수를 테스트가 참조할 수 있게 공개한다:
   ```swift
   /// 한 번의 update가 적분할 수 있는 최대 시간.
   /// 절전에서 깨어나면 수천 초가 들어온다. 그대로 적분하면 파티클이 화면 밖으로
   /// 순간이동하고, 잘게 쪼개 따라잡으면 수만 번 반복하며 배경화면이 멈춘다.
   /// 그래서 넘치는 시간은 **버린다**. 눈이 조금 늦게 내리는 게 멈추는 것보다 낫다.
   public static let maxTimeStep = 0.1
   ```
   `update`는 맨 처음에 이렇게 죈다 — NaN이 비교를 전부 통과해버리므로 먼저 걸러야 한다:
   ```swift
   guard deltaTime.isFinite, deltaTime > 0 else { return }
   let dt = Swift.min(deltaTime, Self.maxTimeStep)
   ```
   비유한값은 방출 시점에도 걸러 렌더러로 새지 않게 한다(위치·속도·크기 전부).
