...
**완료 조건 충족.** `Hiyuki Wutherring Waves`에서 현재 시각이 화면에 보이고
1분 뒤 값이 바뀌는 것을 배경화면 창 캡처로 확인했다(0:25 → 0:26).
이 씬은 그리지 못한 레이어도, 온전하지 않게 그린 레이어도 **0개**다.

보이는 레이어 34개 중 그리지 못하는 것은 4개뿐이고 전부 M6다 —
`_rt_FullFrameBuffer`(렌더 타깃) 3개, 사운드 1개.

**실물로 돌려보고 잡은 결함 넷:**
1. `scriptproperties`를 수만 담아 실물 시계의 `delimiter`(":")를 버렸다.
   화면에 `00undefined22`가 나왔다 — 계획에 예측해 적어둔 그 모양 그대로다.
2. `origin`이 스크립트여도 그 객체의 `value`에 좌표가 있다. 실물 씬 하나는
   origin 스크립트의 `update`가 통째로 주석 처리돼 있어 `value`가 유일한 좌표다.
   이걸 읽는 것만으로 텍스트 3개와 파티클 1개가 풀렸다.
3. 픽셀 포맷 8(RG88)을 몰라 `light_shafts_0`이 떨어졌다. 실물을 재보니
   픽셀당 2바이트에 R은 255 고정, G만 0~228 — 흑백 밝기와 알파다.
4. 오브젝트의 `size`는 글자 크기가 아니라 **상자**다(411x5300짜리도 있다).
   점 크기로 쓰니 글자가 화면 밖으로 밀려났다.

**M5가 남긴 것(M6 이후):**
- `visible`/`alpha`/`scale`/`color`/`origin` 스크립트는 저장된 값으로 그린다.
  Chisa 씬의 여섯 레이어가 여기 해당하는데, 그 스크립트들은 음악 재생기
  이벤트(`mediaThumbnailChanged`)를 요구한다. 텍스트·스크립팅과는 별개의 기능이다.
- `cursor` 레이어의 `input.cursorWorldPosition`(마우스 추종).
- 무한 루프를 **중단**시키지는 못한다. 격리만 한다.
