...
## Task 3 이후 (2026-09-06 저녁)

**자홍색 번짐은 해결됐다.** 체인 입구에서 완전히 투명한 텍셀의 RGB를 지운다
(점 필터 1:1 복사라 그 패스 자체는 번지지 않는다). 번져도 검정만 섞이므로
자홍색보다 훨씬 눈에 안 띈다.

알파를 미리 곱했다가 출구에서 되돌리는 정석도 해 봤는데 **더 나빴다.** 이펙트가
알파를 직접 정하는 경우가 많아(마스크·합성) 그 알파로 나누면 색이 밝아진다.
알파의 의미를 건드리지 않는 쪽이 안전하다는 것을 실물로 배웠다.

**함께 고친 진짜 버그:** `previous`는 **이름 없는 타깃에 그린 결과만** 이어받아야
한다. `_rt_*`는 곁버퍼다. 블러가 이 구분을 요구한다 — 축소·가로·세로는 곁버퍼에
그리고 마지막 합치기가 흐린 곁버퍼와 **원본**을 함께 읽는다.

**예산도 넣었다.** 레이어 텍스처가 화면보다 훨씬 클 수 있어서(9000픽셀짜리가 있다)
체인 하나가 973MB를 잡았다. 작업 해상도의 긴 변을 2560으로 죄고, 씬 전체에
320MB 상한을 뒀다. 973MB → 51MB, 씬 최대 293MB가 됐다.

### 남은 것

- **패스가 여럿이고 곁버퍼를 쓰는 체인이 화면을 하얗게 만든다.** 블러 계열이다.
  패스가 하나인 `foliagesway`는 미리보기와 일치한다. 즉 파이프라인의 기본은
  맞고, 곁버퍼를 오가는 부분이 틀렸다. 다음에 볼 것: 곁버퍼 해상도(1/4)에서의
  뷰포트와 `g_Texture0Resolution`, 그리고 슬롯 1처럼 **바인딩이 없는 슬롯에
  흰색을 묶는 것**이 합치기 셰이더에서 흰색으로 새는지.
- 이펙트가 동작하면 `util/white` 단색 레이어 건너뛰기 규칙을 되돌려야 한다.

## 남은 것 (2026-09-06 저녁, 측정 결과)

이펙트·도형·후처리가 전부 동작한 뒤 남은 미지원은 **`_rt_FullFrameBuffer` 참조 5개**뿐이다.

| 무엇 | 개수 | 필요한 것 |
|---|---|---|
| 오디오 비주얼라이저(Simple Audio Bars) | 3 | 시스템 오디오 캡처(ScreenCaptureKit, 화면 녹화 권한) |
| 커서 레이어 | 1 | 없음 — 4x4 크기에 이펙트가 없어 그려도 배경 4픽셀 복사뿐이다 |
| 합성 레이어 | 1 | 위와 같은 계열 |

### 원근 투영 씬은 별개 마일스톤이다 (근거)

투영 행렬만 넣으면 될 것 같아 실제로 로딩까지 만들어 재 봤다. 결과:

- 카메라는 파일에 정적 값이 있다(`eye 0,0,38`, `fov 53`, `nearz 1`, `farz 10000`).
- 그런데 **레이어 26개 중 그릴 수 있는 것이 6개뿐**이다.
- 못 그리는 넷이 `models/prism/prism.mdl` 같은 **3D 메시**이고, 그게 그 씬의 중심이다
  (`mainPrism`, `prism`).
- 카메라도 정적이 아니다 — `Camera (script)`가 `OMGMatrix`·`OMGVector` 공유 모듈로
  움직인다. 그 스크립트가 `Mat4`를 쓰는데 우리 런타임에 아직 없다.

즉 투영을 넣어도 배경 평면 몇 장만 뜬 깨진 화면이 된다. 미리보기 폴백이 더 낫다.
제대로 하려면 **`.mdl` 로더 + 3D 재질/조명 + `Mat4`와 카메라 스크립팅 API**가 함께
필요하다. 라이브러리 26개 중 1개를 위한 작업이라 우선순위가 낮다.
