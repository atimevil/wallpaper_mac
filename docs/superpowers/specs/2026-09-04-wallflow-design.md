...
**M3 — 움직이는 배경 (assets 참조 + 비디오 텍스처)**
`AssetsStore`(`.pkg`에 없는 참조를 `assets/`에서 해석), 비디오 텍스처 렌더링
(`.tex` 안의 MP4 → AVFoundation → `CVMetalTextureCache`), `solidlayer`(단색 레이어).
*완료 조건: `3536506287`과 `3616103296`이 3200x1800·3840x2160 애니메이션 배경을 재생한다.*

**M4 — 파티클**
파티클 프리셋 모델(emitter 2종, initializer 8종, operator 6종), CPU 시뮬레이션,
Metal 인스턴싱 빌보드 렌더(`genericparticle`의 `GS_ENABLED = 0` 경로).
*완료 조건: `3714517753`에 눈과 벚꽃이 날린다.*

**M5 — 스크립팅과 텍스트**
JavaScriptCore 스크립트 호스트(`createScriptProperties` 셰임, `update(value)` 호출),
폰트 로딩, Core Text 기반 텍스트 레이어.
*완료 조건: 시계와 날짜가 표시되고 `3714517753`이 완전히 재생된다.*

**M6 — 커스텀 셰이더와 이펙트**
GLSL 방언 전처리 → glslang → SPIRV-Cross → MSL 파이프라인, FBO 이펙트 체인
(`composelayer`의 `_rt_FullFrameBuffer` 포함), 오디오 반응.
*완료 조건: 나머지 이펙트가 동작한다. 개별 누락은 허용한다.*

### 마일스톤 재배열 근거 (2026-09-05)

원래 스펙은 M3에 파티클과 텍스트를, M4에 셰이더·오디오·스크립팅을 뒀다.
`assets/` 반입 후 실물을 확인하고 다음 이유로 재배열했다.

1. **실사용 가치 순.** 비디오 텍스처는 가장 적은 일로 가장 많은 화면을 바꾼다 —
   씬 2개가 정지 preview에서 움직이는 4K 배경이 된다. M1의 AVFoundation 경로를
   그대로 재사용하고, 디코더는 이미 MP4를 꺼내준다.
2. **텍스트는 스크립팅 없이 성립하지 않는다.** 보유 씬의 텍스트 레이어 13개 중
   12개가 `"text": {"script": ...}`다. 스크립팅을 뒤에 두면 M3가 완료될 수 없다.
3. **assets 참조 해석이 비디오와 같이 간다.** `3616103296`의 나머지 이미지
   레이어가 `solid_instance_model_*.json` 참조로 실패하는데, 같은 작업으로 풀린다.
